package com.github.wxpay.rabbitmq.routing;
...
public class RoutingProducer {
    public static void main(String[] args) {
        try {
            ...
            channel.exchangeDeclare(RabbitMqUtil.EXCHANGE2,"direct");
            //发布消息到队列中  参数2 消息队列名称 参数4 消息内容
            for (int i = 0; i < 10; i++) {
                String msg="消息"+i+"--->hello rabbbitmq!!!";
                String key=i%2==0?"msg":"error";
                //发布消息到交换机中，参数1，交换机名称；参数2，key
                channel.basicPublish(RabbitMqUtil.EXCHANGE2,key,null,msg.getBytes());
            }
            channel.close();
            connection.close();
        } catch (IOException e) {
